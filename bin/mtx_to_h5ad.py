...
def input_to_adata(
    ...
    if aligner == "cellranger" and not input_data.lower().endswith(".h5"):
        ...
        t2g = pd.read_table(
            feature_file,
            header=None,
            names=["gene_id", "gene_symbol", "feature_types"],
            usecols=[0, 1, 2],
        )
    else:
        if txp2gene:
            t2g = pd.read_table(
                txp2gene, header=None, names=["gene_id", "gene_symbol"], usecols=[1, 2]
            )
        elif star_index:
            t2g = pd.read_table(
                f"{star_index}/geneInfo.tab",
                header=None,
                skiprows=1,
    ...
    if (
        txp2gene
        or star_index
        or (aligner == "cellranger" and not input_data.lower().endswith(".h5"))
    ):
...
def write_counts(
    ...
    pd.DataFrame(adata.obs.index).to_csv(
        os.path.join(out, "barcodes.tsv"), sep="\t", index=False, header=None
    )
    pd.DataFrame(adata.var).to_csv(
        os.path.join(out, "features.tsv"), sep="\t", index=True, header=None
    )
    io.mmwrite(os.path.join(out, "matrix.mtx"), adata.X.T, field="integer")

    if verbose:
        print(
            "Wrote features.tsv, barcodes.tsv, and matrix.mtx files to {}".format(
                args["out"]
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--input_data",
        dest="input_data",
        help="Path to either mtx or mtx h5 file.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--task_process", dest="task_process", help="Task process name."
    )
    parser.add_argument(
        "--txp2gene",
        dest="txp2gene",
        help="Transcript to gene (t2g) file.",
        nargs="?",
        const="",
    )
    parser.add_argument(
        "--star_index",
        dest="star_index",
        help="Star index folder containing geneInfo.tab.",
        nargs="?",
        const="",
    )
